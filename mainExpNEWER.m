...
cpd_task = 1.5;
cpd_adaptor = 1.5;
reference_size = task_size; 
cps_reference = cpd_task*reference_size;
cpd_random = 3;
cps_adaptor = cpd_adaptor*adaptorsize;
cps_task = cpd_task*task_size;
...
%numoftrials = 10;
 ...
 %RANDOM FRAME TO PRESENT MULTIPLE SF
...
offsetpixdown = adaptorsizepixel-offsetpix; %95 pixels if on laptap
offsetpixup = -1.*offsetpixdown;
        ...
        for i=1:numPhases
            ...
            if updown == 1
                adaptorGrating = drawGrating(gratAng(updown),adaptorsizepixel,...
                    cps_adaptor,0,gabWidthAdaptor,maskSizeAdaptor,phase);
            else
                adaptorGrating = drawGrating(gratAng(updown),adaptorsizepixel,...
                    cps_adaptor,0,gabWidthAdaptor,maskSizeAdaptor,phase+pi);
            end
            randomGrating = drawGrating(gratAng(updown),adaptorsizepixel,...
                cps_random,0,gabWidthAdaptor,maskSizeAdaptor,phase);
...
if train == 1
    ...
    while trainBlockIT <= trainBlockNum
        while ~Start
            if trainBlockIT == 0
                DrawFormattedText(w,['\n\n\n\n\n\n\nPlease Fixate at the Fixation Point During The Experiment\n'...
                    'Press RETURN to start experiment'],'center','center',black);
            else
                blockRemain = num2str(trainBlockNum-trainBlockIT);
                DrawFormattedText(w,['\n\n\n\n\n\n\nPlease Fixate at the Fixation Point During The Experiment\n'...
                    'Press RETURN to start experiment\n',blockRemain,' Blocks Remaining'],'center','center',black);
            end
        ...
        while trainBlockEnd - trainBlockStart < dur_train
            ...
            testGrating = drawGrating(testRandomOrt,refsizepixel,cps_reference,0,gabWidthAdaptor...
                ,maskSizeTask,0);
            test_tex = Screen('MakeTexture', w, round(gray+taskCtrst*testGrating));
            
            %TASK GRATING
            matchGrating = drawGrating(matchRandomOrt,tasksizepixel,cps_task,0,gabWidthAdaptor...
                ,maskSizeTask,0);
            ...
            while breakLoop == 0
                ...
                while GetSecs - timeStamp <= 0.25 && breakLoop == 0
                    ...
                    task_grating_change = drawGrating(ort_change,tasksizepixel,...
                        cps_task,0,gabWidthTask,maskSizeTask,0);
...
while baseLineEnd - baseLineStart < dur_Baseline
    ...
    testRandomOrt = 0;
    %TestOrt(end+1) = testRandomOrt;
    randphase_task = 2.*pi.*rand(1);
    randphase_probe = 2.*pi.*rand(1);
    %TASK GRATING
    matchGrating = drawGrating(matchRandomOrt,tasksizepixel,cps_task,0,gabWidthTask...
        ,maskSizeTask,randphase_task);
    testGrating = drawGrating(testRandomOrt,refsizepixel,cps_reference,0,gabWidthTask...
        ,maskSizeTask,randphase_probe);
    test_tex = Screen('MakeTexture', w, round(gray+taskCtrst*testGrating));
    match_tex = Screen('MakeTexture', w, round(gray+taskCtrst*matchGrating));
    ...
    while breakLoop == 0
        ...
        while GetSecs - timeStamp <= 0.25 && breakLoop == 0
            ...
            if buttons(1) == 1 && GetSecs-baseLineEnd >= 0.2
                ...
                SetMouse(xCenter,yCenter,w);
                Screen('FillOval', w, white, fixRect1);
                Screen('FillOval', w, black, fixRect2);
                Screen('Flip',w);
                baseLineEnd = GetSecs;
                breakLoop = 1;
        ...
        while GetSecs - timeStamp2 <= 0.1 && breakLoop == 0
            ...
            if buttons(1) == 1 && GetSecs-baseLineEnd >= 0.2
                TAE_base(end+1) = [ort_change];
                Screen('FillOval', w, gray, fixRect1);
                Screen('FillOval', w, white, fixRect2);
                Screen('Flip',w);
                taskTime_base(end+1) = GetSecs-baseLineStart;
                SetMouse(xCenter,yCenter,w);
                baseLineEnd = GetSecs;
        ...
        while GetSecs - timeStamp <= 0.25 && breakLoop == 0
            ...
            poppop(delta_ort,sndInfo);
            task_grating_change = drawGrating(ort_change,tasksizepixel,...
                cps_task,0,gabWidthTask,maskSizeTask,randphase_task);
            task_tex_change = Screen('MakeTexture', w, round(gray+taskCtrst*task_grating_change));
            Screen('DrawTexture', w, task_tex_change,[],imgRect_Match);
            %Screen('DrawTexture', w, test_tex,[],imgRect_Test);
            %DrawFormattedText(w,['\n\n\n\n\n\n\n\n\n\n\n\nCurrent Ort is at\n',txtOrt,'degrees\n\n\n\n Current OLDOrt is at\n',txtOldOrt,'degrees\n\n\n\n Current DELTAOrt is at\n',txtOrtChange,'degrees\n\n\n\n'],'center','center',white);
...
while ~StartAdapt
    DrawFormattedText(w,['\n\n\n\n\n\n\nPlease Fixate at the Fixation Point During The Experiment\n'...
        'Press RETURN to continue'],'center','center',black);
    Screen('FillOval', w, gray, fixRect1);
    Screen('FillOval', w, black, fixRect2);
    Screen('Flip',w);
    [KeyDown,time,keyCode] = KbCheck;
...
while expEndTimeStamp - expStartTimeStamp < dur_Block
    ...
    movieFrameIndicesUP = randi(numPhases,movieDurationFrames,1);
    movieFrameIndicesDown = randi(numPhases,movieDurationFrames,1);
    
    %IM N-BACK TASK:
    randNum = rand;
    
    if randNum <= nBackProb
        nBack = 1;
    else
        nBack = 0;
    end
    
    updown = randi(2); randomFrame = randi([4, 30]);
    for i=1:movieDurationFrames
        %for updown = 1:2
            if i == randomFrame
                %Screen('DrawTexture', w, tex(updown, movieFrameIndices(i)),[],imgRect_Adpt(updown,:));
                Screen('DrawTexture',w,RandomGratingtex(updown, movieFrameIndicesUP(i))...
                    ,[],imgRect_Adpt(updown,:));
            else
%                 if updown == 1
                    Screen('DrawTexture', w, tex(1, movieFrameIndicesUP(i)),[],imgRect_Adpt(1,:));
%                 else
                    Screen('DrawTexture', w, tex(2, movieFrameIndicesDown(i)),[],imgRect_Adpt(2,:));
%                 end
            end
        %end
    ...
    while breakLoop == 0
        ...
        if im == 1
            ...
            %[NormedImg] = normImageNEW(randomImgMat,RMSCtrst,wantSize,testgabWidth);
            %NormedImg(:,:,2) = 255.*imageGaus; %255 is 0 transparency
            if nBack == 1 && trial_num ~= 1
                testtex = Screen('MakeTexture', w, postImg{trial_num-1});
                taskIM(end+1) = 1;
            else
...
%                 task_tex_change = Screen('MakeTexture', w, round(gray+taskCtrst*task_grating_change));
        ...
        timeStamp2 = GetSecs;
        while GetSecs - timeStamp2 <= 0.25 && breakLoop == 0 &&...
                GetSecs - expEndTimeStamp > 0.25
            [ort_change,delta_ort,old_ort] = ORTchange(matchRandomOrt,dpp,xCenter,w,ort_change,delta_ort,old_ort);
            %txtOrt = num2str(ort_change);
            %txtOldOrt = num2str(old_ort);
        ...
        %TOPUP: 2000MS
        if breakLoop == 0
            movieDurationSecs = topUpInTrial;
            movieDurationFrames=round(movieDurationSecs./frameDur);
            
            movieFrameIndicesUP = randi(numPhases,movieDurationFrames,1);
            movieFrameIndicesDown = randi(numPhases,movieDurationFrames,1);
            
            updownTopUp = randi(2); randomFrameTopUp = randi([4, 16]);
            
            for i=1:movieDurationFrames
                if i == randomFrameTopUp
                    %Screen('DrawTexture', w, tex(updown, movieFrameIndices(i)),[],imgRect_Adpt(updown,:));
                    Screen('DrawTexture',w,RandomGratingtex(updownTopUp, movieFrameIndicesUP(i))...
                        ,[],imgRect_Adpt(updownTopUp,:));
                     timeSF(end+1) = GetSecs-expStartTimeStamp;
...
clear imgData; clear tempdata; clear randomImgCell; clear randomImgMat; clear NormedImg; clear imageGaus; clear tempNormIMG; clear postImg; clear imgX; clear imgY; clear testGrating; clear task_grating_change; clear randomGrating; clear matchGrating; clear adaptorGrating;  
save(resultFilePath);
...
%% End Prompt
EndEXP = GetSecs;
while GetSecs - EndEXP < 2
    DrawFormattedText(w,['\n\n\n\n\n\n\nThe Experiment has Ended\n'...
        'Thank you for your participation :-)'],'center','center',black);
    Screen('FillOval', w, gray, fixRect1);
    Screen('FillOval', w, black, fixRect2);
    Screen('Flip',w);
end
